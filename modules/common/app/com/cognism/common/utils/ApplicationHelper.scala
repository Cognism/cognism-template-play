...
object ApplicationHelper {
  ...
  def isSameDayMonthYear(time1:Long, time2:Long):Boolean = {
    ...
    date1.year().get()==date2.year().get() && date1.monthOfYear().get()==date2.monthOfYear().get() && date1.dayOfMonth().get()==date2.dayOfMonth().get()
  }
  ...
  case class Timer(name:String){
    ...
    private val timeblocks = scala.collection.mutable.Map.empty[String,(Int,Int)]
    ...
    def time[A](name: String)(block: => A) = {
      ...
      timeblocks.get(name) match {
        case Some((duration,counter)) =>
          timeblocks.put(name, (duration + ((System.nanoTime-s)/1e6).toInt, counter + 1))
        case _ =>
    ...
    override def toString():String = {
      ...
      val buffer = new StringBuilder(s"[TIMER] $name - took ${totalExecutionTime}ms => ")

      timeblocks.foreach{ case(blockname, (totaltime,counter)) =>
        buffer.append(s"$blockname=${totaltime}ms[avg=${(totaltime/counter)}ms] ")
      }
